...
const DEFAULT_AVATAR = 'https://images.unsplash.com/photo-1592924357228-91a4daadcfea?auto=format&fit=crop&w=100&q=80';
...
const ShopOrders = () => {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        const res = await ordersAPI.getByShopkeeper(shopkeeperId);
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      ...
      if (res.success) {
        setOrders(orders.map(o => (o._id === id ? res.order : o)));
        if (selectedOrder?._id === id) setSelectedOrder(res.order);
  ...
  const filteredOrders = orders.filter(order => {
    const customerName = order.farmer?.name || '';
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredOrders.length > 0 ? filteredOrders.map((order) => (
                <tr key={order._id} className="hover:bg-slate-50/80 transition-colors group">
                  ...
                  <td className="p-5">
                    <div className="flex items-center gap-3">
                      <img src={DEFAULT_AVATAR} alt="User" className="w-8 h-8 rounded-full object-cover border border-slate-200" />
                      <span className="text-sm font-semibold text-slate-800">{order.farmer?.name || 'Farmer'}</span>
                    </div>
                  </td>
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      {order.paymentMethod === 'Online' && (
                        <span className={`text-[10px] font-semibold ${order.paymentStatus === 'Paid' ? 'text-emerald-600' : 'text-amber-600'}`}>
                          {order.paymentStatus === 'Paid' ? '✓ Payment Done' : 'Payment Pending'}
                        </span>
                      )}
                    </div>
                  </td>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <div>
                ...
                <p className="text-xs text-slate-500">{selectedOrder.orderId || selectedOrder._id} • {formatDate(selectedOrder.createdAt)}</p>
              </div>
              <button
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-3">
                <div className="flex items-center gap-3 mb-2">
                  <img src={DEFAULT_AVATAR} alt="User" className="w-12 h-12 rounded-full object-cover border-2 border-white shadow-sm" />
                  <div>
                    <h4 className="font-bold text-slate-800">{selectedOrder.farmer?.name || 'Farmer'}</h4>
                    <div className="flex flex-wrap gap-1 mt-1">
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold border ${getStatusStyle(selectedOrder.status)}`}>
                        {selectedOrder.status}
                      </span>
                      {selectedOrder.paymentMethod === 'Online' && (
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${selectedOrder.paymentStatus === 'Paid' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                ...
                <div className="grid grid-cols-1 gap-2 text-sm text-slate-600">
                  ...
                  <div className="flex items-center gap-2">
                    <MapPin size={14} className="text-slate-400" /> {selectedOrder.shippingAddress}
                  </div>
                </div>
              </div>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-100 flex gap-3">
              ...
              {selectedOrder.status === 'Pending' && (
                <button
                  onClick={() => { handleStatusChange(selectedOrder._id, 'Completed'); setSelectedOrder(null); }}
