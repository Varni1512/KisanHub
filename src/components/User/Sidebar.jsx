  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 px-4 space-y-1">
        {menuItems.map((item) => (
          <button
            ...
            className={`w-full flex items-center cursor-pointer gap-3 px-4 py-3.5 rounded-xl text-sm font-semibold transition-all ${
              activeTab === item.name 
      ...
      <div className="p-4 border-t border-slate-100">
        <button className="flex items-center gap-3 cursor-pointer text-red-500 w-full px-4 py-3 hover:bg-red-50 rounded-xl transition-colors font-semibold">
          <LogOut size={20} />
          Logout
        </button>
      </div>
